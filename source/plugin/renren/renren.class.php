...
class plugin_renren_forum extends plugin_renren {
	...
	function viewthread_postfooter_output() {
		global $_G, $postlist;
		if(!$_G['cache']['plugin']['renren']['rr_thread_like']&&(empty($_G['gp_page']) || $_G['gp_page'] == 1)) {
			$i=0;
	...
	function viewthread_sidetop_output() {
		global $_G, $postlist;
		if($_G[tid]&&!$_G['cache']['plugin']['renren']['rr_thread_side_top']) {
			require_once './source/plugin/renren/renren.function.php';
			$i=0;
			foreach($postlist as $key=>$a) {
	...
	function post_to_renren_aftersubmit_output() {
		...
		if($_G['renren']['rr_uid'] && $_G['cache']['plugin']['renren']['rr_pic']) {
			...
			$iffeed = get_rrfeed($_G['uid']);
			//假如是发主题贴
			if( $_G['gp_action'] == 'newthread' && (submitcheck('topicsubmit', 0, $GLOBALS['seccodecheck'], $GLOBALS['secqaacheck']) ) && $_POST['rrfeed']&&$_G['cache']['plugin']['renren']['rr_feednew']) {
			...
			//假如是发回复
			} elseif( $_G['gp_action'] == 'reply' && (submitcheck('replysubmit', 0, $GLOBALS['seccodecheck'], $GLOBALS['secqaacheck']) ) && ($_POST['rrfeed'] || $iffeed[1])&&$_G['cache']['plugin']['renren']['rr_feedre']) {
...
//同步记录到人人网状态按钮位置
	...
	function spacecp_doing_aftersubmit_message($param){
		...
		$renren = & renren_connect::instance();
		$renren->inits($discuz_uid);
		if( $_G['renren']['rr_uid']&&getgpc('addsubmit') && isset($param['param'][2]['doid']) && $param['param'][2]['doid'] > 0&&substr($param['param'][0], -8) =='_success'&&$_POST['rrfeed']&&$_G['cache']['plugin']['renren']['rr_statusnew'])
		{
...
//分享相册
	...
	function spacecp_share_aftersubmit_message($param){
		...
		$renren = & renren_connect::instance();
		$renren->inits($discuz_uid);
		if($_G['cache']['plugin']['renren']['rr_sharenew']&&getgpc('sharesubmit')&&isset($param['param'][2]['sid'])&&$param['param'][2]['sid']>0&&$_G['renren']['rr_uid']&&substr($param['param'][0], -8)=='_success')
		{
			require_once './source/plugin/renren/class/api.class.php';
			require_once './source/plugin/renren/renren.function.php';
			$iffeed = get_rrfeed($_G['uid']);
...
//日志同步
	function spacecp_blog_sync_to_renren_aftersubmit_message(){
		...
		$renren = & renren_connect::instance();
		$renren->inits($discuz_uid);
		if($_G['renren']['rr_uid'] &&$_G['cache']['plugin']['renren']['rr_blognew']) {
			require_once './source/plugin/renren/class/api.class.php';
			require_once './source/plugin/renren/renren.function.php';
			require_once './source/function/function_post.php';
			$feed = & renren_api::instance();
...
class plugin_renren_portal extends plugin_renren{
	...
	function portalcp_article_sync_to_renren_aftersubmit_output(){
		...
		$renren = & renren_connect::instance();
		$renren->inits($discuz_uid);
		if($_G['renren']['rr_uid'] && $_G['cache']['plugin']['renren']['rr_articalnew']) {
			require_once './source/plugin/renren/class/api.class.php';
			require_once './source/plugin/renren/renren.function.php';
			require_once './source/function/function_post.php';
			$feed = & renren_api::instance();
