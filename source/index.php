...
$user_a = DB::query("SELECT * FROM ".DB::table('user_type ')."order by displayorder desc");
...
$conditionsql.="u.typeid=$user_type and ";
}
//增加身高条件
...
$conditionsql.=" m.groupid=21 ";//限制搜索艺人会员用户组
//echo $conditionsql;
...
//echo "$modurl?mod=list&filter=$_GET[filter]$catedisplayadd[order]$catedisplayadd[searchoption]";
	$query = DB::query("SELECT m.uid,t.name,m.username,COUNT(*) as count  FROM  ".DB::table('common_member')." as m left join ".DB::table('user_actor_type')."  u  on m.uid=u.uid left join ".DB::table('common_member_profile')."  p on p.uid=m.uid left join ".DB::table('user_type')." t on u.typeid=t.id where $conditionsql  group by u.uid  order by uid desc $limit ");
	$jobi=0;
