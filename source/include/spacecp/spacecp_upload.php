...
if(submitcheck('albumsubmit') && helper_access::check_module('album')) {
	...
	if($_POST['starAddPhoto'] === '1'){ // 2013.5.29 用于从明日之星的上传跳转到展示
		showmessage('您己经成功上传照片','/events.php?mod=three');
	}else{ //普通上传的跳转

		if($_POST['bkid']){
			echo "<script>window.location.href='/home.php?mod=ucenter&do=setbkpic&spaceuid=".$_G['uid']."&bkid=".$_POST['bkid']."'</script>";
			exit;
		}
		
		if($_POST['albumid']){
			showmessage('upload_images_completed', "home.php?mod=ucenter&do=albumphotos&aid={$_POST['albumid']}");
		}

		
//		showmessage('upload_images_completed', "home.php?mod=space&uid=$_G[uid]&do=album&quickforward=1&id=".(empty($albumid)?-1:$albumid));
		showmessage('upload_images_completed', "home.php?mod=ucenter&do=album");
