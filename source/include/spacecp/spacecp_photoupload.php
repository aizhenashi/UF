...
DB::query("update ".DB::table('common_member')." set status=8 where uid={$_G['uid']}");
...
if($_GET['click8']){
	$d=DB::fetch_first("select picid from ".DB::table('home_pic')." where uid = {$_G['uid']} ORDER BY picid desc limit 1");
	$mylastid = $d['picid'];
	DB::query("update ".DB::table('home_pic')." set click8 = '1' where picid = {$mylastid}");	
	showmessage('您己经成功上传照片');
}
include_once template("home/spacecp_photo_upload");
