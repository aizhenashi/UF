...
/*
*图片上传部分
*1，上传图片，存表6个URL，生成缩略图1（370*205，上传页显示），缩略图2（235*155，首页显示），缩略图3（238*159，列表页显示），缩略图4（559*396，详情页显示），缩略图5（190*105，搜索结果页），原型图片
*2，从原始图片获取图片的基本信息（函数）
*3，获取表单传递的数据
*4，update写入图片信息
*
*/
if(empty($_G['uid'])){
	showmessage("请登录您的帐号","/login.html");
}
...
if(!empty($_POST)){
	...
	$lasttime = time();//上传时间
	$mime_type = explode('/',$_POST['picmime']);
	$mime = $mime_type[1];
	//var_dump($mime_type);
	$xia = 0;
	...
	if(!empty($title)){
		$res = DB::query("insert into ".DB::table("creation_views")."(id,uid,title,city,address,pictime,deal,type,price,url1,url2,url3,url4,url5,url,lasttime,size,width,height,xia,mime) values(null,{$_G['uid']},'{$title}',{$place_id},'{$xiangxidizhi}',{$pictime},{$check},{$type},{$price},'{$pic370}','{$pic235}','{$pic238}','{$pic559}','{$pic190}','{$picfile}',{$lasttime},{$picsize},{$picwidth},{$picheight},{$xia},'{$mime}')");

		showmessage("上传成功","/creation/");
	}
}
//echo strtotime("2013-09-23 00:00:00");
//var_dump($_POST);
include template('creation/picUpload');
?>