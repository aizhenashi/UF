...
$arr = explode('~',$weight_str);

$minWeight = $arr[0] != 'd' ? $arr[0] : '';
$maxWeight = $arr[1] != 'u' ? $arr[1] : '';

$dbool = preg_match("/d~/",$weight_str);
$ubool = preg_match("/~u/",$weight_str);
$bool = $dbool == '1' ? 'down' : ($ubool == '1' ? 'up' : 'no');

switch ($bool){
	case 'down' : 
		$weight_str = str_replace('d~','',$weight_str);
		$weight_str .= "KG以下";
		break;
	case 'up' :
		$weight_str = str_replace('~u','',$weight_str);
		$weight_str .= 'KG以上';
		break;
	default:  
		$weight_str .= 'KG';		
...
$arr = explode('~',$height_str);
$minHeight = $arr[0] != 'd' ? $arr[0] : '';
$maxHeight = $arr[1] != 'u' ? $arr[1] : '';

$dbool = preg_match("/d~/",$height_str);
$ubool = preg_match("/~u/",$height_str);
$bool = $dbool == '1' ? 'down' : ($ubool == '1' ? 'up' : 'no');

switch ($bool){
	case 'down' : 
		$height_str = str_replace('d~','',$height_str);
		$height_str .= "CM以下";
		break;
	case 'up' :
		$height_str = str_replace('~u','',$height_str);
		$height_str .= 'CM以上';
		break;
	default:  
		$height_str .= 'CM';		
...
if(!empty($_GET['age'])&&$_GET['age']!="all"&&!in_array($_GET['age'],array_keys($user_age['choices'])))
{
	...
	$arr = explode('~',$age_str);
	$minAge = $arr[0] != 'd' ? $arr[0] : '';
	$maxAge = $arr[1] != 'u' ? $arr[1] : '';
	
	$dbool = preg_match("/d~/",$age_str);
	$ubool = preg_match("/~u/",$age_str);
	$bool = $dbool == '1' ? 'down' : ($ubool == '1' ? 'up' : 'no');
	
	switch ($bool){
		case 'down' : 
			$age_str = str_replace('d~','',$age_str);
			$age_str .= "岁以下";
			break;
		case 'up' :
			$age_str = str_replace('~u','',$age_str);
			$age_str .= '岁以上';
			break;
		default:  
			$age_str .= '岁';		
...
$conditionsql.=" m.groupid=21 and t.topid <>'81' ";//限制搜索艺人会员用户组
//echo $conditionsql;
...
$sortdata['count'] = DB::num_rows(DB::query("SELECT COUNT(*)  FROM  ".DB::table('common_member')." as m left join ".DB::table('user_actor_type')."  u  on m.uid=u.uid left join ".DB::table('common_member_profile')."  p on p.uid=m.uid left join ".DB::table('user_type')." t on u.typeid=t.id where $conditionsql  group by u.uid"));
...
include template('diy:talents/index');
