...
if($usergroupid != 22)
{
	showmessage(lang('hr/template', 'job_usergroup_nopur'),'home.php');
}
...
if ( $action == 'job'){
	...
	if($type == "list"){
		//招聘列表
		$job_list = array();
		$nums=DB::fetch_first("SELECT count(*) as num FROM ".DB::table('hr_recruitment')." r LEFT JOIN ".DB::table('common_member')." u ON r.uid=u.uid WHERE r.uid=".$_G['uid']);
		$count=$nums['num'];
		$job_query = DB::query("SELECT r.id as id, r.uid as uid, title, username, posttime, description FROM ".DB::table('hr_recruitment')." r LEFT JOIN ".DB::table('common_member')." u ON r.uid=u.uid WHERE r.uid=".$_G['uid']." limit $start,$perpage");
		while($job_row = DB::fetch($job_query)){
			$job_list[] = $job_row;
		}
		$multi = multi($count, $perpage, $page, "user.php?mod=post&action=job&type=list");
		//print_r( $job_list);
		include template('diy:user/hr_job_list');
	}elseif($type == "edit"){
...
//print_r($job_detail);
		if (submitcheck('profilesubmitbtn')){
			...
			showmessage('编辑成功', 'user.php?mod=post&action=job&type=list');
		}else{
			$action_url = "user.php?mod=post&action=job&type=edit";
			...
			include template('diy:user/hr_post_job');
		}
	}elseif($type == "del"){
	
			DB::query("DELETE FROM ".DB::table('hr_recruitment')." WHERE id=".$_G["gp_jobid"]);
			showmessage('删除成功', 'user.php?mod=post&action=job&type=list');
		...
		if(!empty($_POST)){
			...
			if(in_array("",$arr)){
					//var_dump($_POST);
					echo '<script type="text/javascript">alert("请您填写详细信息");window.location.href="user.php?mod=post&action=job";</script>';
					exit();
		}
		}
		...
		if (submitcheck('profilesubmitbtn')){
			...
			showmessage('发布成功,请等待管理员审核', 'user.php?mod=post&action=job&type=list');
		}else{
			$action_url = "user.php?mod=post&action=job";
			/*初始地区数据*/
