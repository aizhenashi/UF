...
if($_GET['operation'] == 'upload') {
	...
	if(helper_access::check_module('album')) {
		...
		if($_FILES["Filedata"]['error']) {
			...
			if($_GET['bkid']){ //м╪ф╛╟Ф©Иио╢╚м╪ф╛

				$id = c::t('home_picbk_pic')->addPic($_GET['bkid'],$file['picid'],$file['filepath']);
				if(!$id){
					$notsave = 1;
				}
			...
			if(!empty($file) && is_array($file)) {
				...
				if($_GET['bkid'] && $notsave == '1'){
					$string = "{\"picid\":\"$file[picid]\", \"url\":\"$url\", \"bigimg\":\"$bigimg\", \"xiezhennotsave\":\"1\"}";
					
				}else{

					$string = "{\"picid\":\"$file[picid]\", \"url\":\"$url\", \"bigimg\":\"$bigimg\"}";
				}
