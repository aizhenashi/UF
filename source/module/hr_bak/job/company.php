...
$gid = intval($_GET['gid']);
$actionarray = array('list', 'store', 'cpapplication');
...
if($action == 'list') {
	...
	$groupnum = DB::result_first("SELECT COUNT(*) FROM ".DB::table('hr_'.$modidentifier.'_usergroup')." WHERE type='company' AND verify='1'");
	$multipage = multi($groupnum, $shownum, $page, "$modurl?mod=company");
	
	$query = DB::query("SELECT * FROM ".DB::table('hr_'.$modidentifier.'_usergroup')." WHERE type='company' AND verify='1' ORDER BY displayorder DESC LIMIT $start_limit, $shownum");
	while($group = DB::fetch($query)) {
		$usergrouplist[$group['gid']]['banner'] = $group['banner'] ? get_logoimg($group['banner']) : './static/image/job/defaultcompanylogo.gif';
		$usergrouplist[$group['gid']]['title'] = $group['title'];
		$usergrouplist[$group['gid']]['type'] = $group['type'];
	}

	include template('diy:hr/'.$modidentifier.'_company:'.$gid);
	...
	$navtitle = $usergroup['title'].' - 合作企业 - '.$channel['title'];
	...
	$multipage = multi($_G['hr_threadcount'], $_G['tpp'], $page, "$modurl?mod=company&action=store&sortid=$sortid&gid=$gid&cid=$cid");
	...
	if(!submitcheck('applicationsubmit')) {
		...
		$newcompany = isset($_GET['newcompany']) ? dhtmlspecialchars(censor(trim($_GET['newcompany']))) : '';
		$groupid = empty($newcompany) ? intval($_GET['usergroup']) : 0;
		...
		if($newcompany) {
			...
			//给管理员发信息
			$msgtitle = $realname.'申请创建合作企业“'.$newcompany.'”，请到后台审核';
			foreach($adminids as $touid) {
				sendpm($touid, $msgtitle, $msgtitle, 0);
			}
		}
		...
		//给管理员发消息
		$msgtitle = $realname.'申请加入“'.$newcompany.'”，请到后台审核';
		foreach($adminids as $touid) {
			sendpm($touid, $msgtitle, $msgtitle, 0);
		}
	...
	include template('diy:hr/'.$modidentifier.'_cpapplication');
