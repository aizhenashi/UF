...
class table_home_follow extends discuz_table
{
	...
	/**
	 * 通过ziduan, where, limit,order 获取数据 
	 */
	public function getdata($ziduan, $where = '1=1', $order = '`dateline` desc', $limit = ''){
		return DB::fetch_all("SELECT {$ziduan} FROM %t where $where ORDER BY {$order} $limit", array($this->_table));
	}
}
