...
Class discuz_upload{
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		if($type == 'album' || $type == 'forum' || $type == 'portal' || $type == 'category' || $type == 'profile' || $type= 'active') {
			$subdir1 = date('Ym');
			$subdir2 = date('d');
			$subdir = $subdir1.'/'.$subdir2.'/';
	...
	function check_dir_type($type) {
		return !in_array($type, array('forum', 'group', 'album', 'portal', 'common', 'temp', 'category', 'profile','active')) ? 'temp' : $type;
	}
