...
class Images{
	...
	function move_uploaded(){        //移动上传文件
		...
		global $_G;
		$sql = "INSERT INTO `uestar`.`pre_temp_touxiang_pic` ( `uid` ,`uname` ,`res_touxiang` ,`time` ) VALUES (  '".$_G['uid']."', '".$_G['usernames']."', '".$this->saveDir."/".$file_newname."', '".time()."')";
		DB::query($sql);
		...
		if($result){
			...
			$newpath="uploads/".date("YmdHis").rand(1000,9999).".jpg";
            $this->ImageToJPG($path,$newpath,$imginfo['width'],$imginfo['height']);
			return $newpath;
		}else{
    ...
    function thumb($image,$is_save=true,$suofang=0,$type='',$maxWidth=445,$maxHeight=350,$interlace=true){
        // 获取原图信息
       $info  = $this->getImageInfo($image);
         if($info !== false) {
			...
			//imagejpeg($srcImg,'uploads/fdsafdasf.jpg',100);
            ...
            if ($suofang==1) {
				...
				$thumbname02 = substr_replace($image,"02.".$type,$length);		//中头像
				$thumbname03 = substr_replace($image,"03.".$type,$length);		//小头像
                ...
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],200,200,$_POST['w'],$_POST['h']);
				
				$thumbImg02 = imagecreatetruecolor(100,100);
                imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],100,100,$_POST['w'],$_POST['h']);

				$thumbImg03 = imagecreatetruecolor(40,40);
                imagecopyresampled($thumbImg03,$thumbImg,0,0,$_POST['x'],$_POST['y'],40,40,$_POST['w'],$_POST['h']);
...
//				unlink($image);
				...
				return array('big' => $thumbname01 , 'middle' => $thumbname02,'small'=>$thumbname03);	//返回包含大小头像路径的数组
            }else{
...
$im =$this->imagetranstowhite($im,128,128);
break; 
...
function imagetranstowhite($trans,$nw,$nh) {
 ...
 imagecopyresized($white, $trans, 0, 0, 0, 0,$nw, $nh, $w, $h);
 return $white;
}
}
///***************
