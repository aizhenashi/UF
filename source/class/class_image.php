...
class image {
	...
	function Thumb($source, $target, $thumbwidth, $thumbheight, $thumbtype = 1, $nosuffix = 0) {
		...
		//如果没有传缩略图的高度 或者 缩略图的高度大于源图片的高度
		if(!$thumbheight || $thumbheight > $this->imginfo['height']) {
			//获取缩略图的高度
			//缩略图的宽度 大于图片的宽度
				//缩略图的高度 就取原图的高度
			//缩略图的宽度 小于或者等于原图的高度
				//$this->imginfo['height']*($thumbwidth/$this->imginfo['width'])
				//缩略图的高度 就取 原图的高度*（缩略图的宽度/原图的宽度）
			$thumbheight =  $this->imginfo['height']*($thumbwidth/$this->imginfo['width']);
		}
		//缩略图的高度赋值
	...
	function init($method, $source, $target, $nosuffix = 0) {
		 ...
		 *  下标 0 宽 不带单位
		 *  下标1 高 不带单位
		 *  下标2 1=GIF 2=JPG 3=PNG
		 *  下标3 宽高的字符串 width="550" height="677"
		 *  下标bits 给出的是图像的每种颜色的位数,二进制格式
		 *  下标channels 给出的是图像的通道值 ,RGB
		 *  下标 mime 文件的mime类型
		 */
		...
		switch($this->param['thumbtype']) {
			...
			case 3:
				if($this->imginfo['width'] >= $this->param['thumbwidth'] || $this->imginfo['height'] >= $this->param['thumbheight']) {
					...
					$this->Set_Offset();
					$thumb_photo = imagecreatetruecolor($this->param['thumbwidth'], $this->param['thumbheight']);
					imagecopyresampled($thumb_photo, $attach_photo ,0, 0, $this->offsetx, $this->offsety, $this->param['thumbwidth'], $this->param['thumbheight'], $cx, $cy);
	...
	function Get_SxSy(){
		...
		//获得高与宽的比例
		$Bili = $this->param['thumbheight']/$this->param['thumbwidth'];
		//计算其高
		if($Bili > 1){

			$guessGao = floor($this->imginfo['width'] * $Bili);
			$guesskuan =  $this->imginfo['width'];
			if($guessGao < $this->imginfo['height']){//猜测成立
				$realHeight = $guessGao;
				$realWidth = $guesskuan;
			}else{//猜测不成立
				$realHeight = $this->imginfo['height'];
				$realWidth = floor($this->imginfo['height']/$Bili);
			}
		}else{ //宽长 计算其宽
			$Bili = $this->param['thumbwidth']/$this->param['thumbheight'];
			
			$guesskuan = floor($this->imginfo['height']*$Bili);
			$guessGao = $this->imginfo['height'];
			
			if($guesskuan < $this->imginfo['width']){ //猜测成立
				$realWidth = $guesskuan;
				$realHeight = $guessGao;
				
			}else{ //猜测不成立

				$realWidth = $this->imginfo['width'];
				$realHeight = floor($this->imginfo['width']/$Bili);
			}
	...
	/*
	 * 设置偏移量
	 * 切正方形图片 当图片不成比例时会使用这个函数
	 * 设置 原图 的x y 轴 偏移量
	 */
	function Set_Offset(){
		
		$this->offsetx = floor(($this->imginfo['width']-$this->cx)/2);
		$this->offsety = floor(($this->imginfo['height']-$this->cy)/2);
