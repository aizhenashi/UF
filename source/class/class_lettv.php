...
class LetvCloudV1 {
		...
		public $user_unique = '664805680f'; 
		public $secret_key = '342d9930141805905a057aad7c28f04e';
         ...
         * @return string
         */
        public function videoUpload($video_name,$video_file){
        	$api = 'video.upload.init';
        	$params['video_name'] = $video_name;
        	$params['client_ip'] = $_SERVER['REMOTE_ADDR'];
        	$params['file_size'] = filesize($video_file);
        	$res = self::httpCall($api, $params);
        	$res_arr = json_decode($res,true);
        	if($res_arr['code'] == 0){
        		$upload_url = $res_arr['data']['upload_url'];
        		$postFields['video_file'] = "@".$video_file;
        		$res_up = $this->curl($upload_url,$postFields);
        		return $res_up;
        	}else{
        		return $res;
        	}
