...
Class discuz_upload{
	...
	function save($ignore = 0) {
		...
		if(empty($this->attach) || empty($this->attach['tmp_name']) || empty($this->attach['target'])) {
			$this->errorcode = -101;
		} elseif(in_array($this->type, array('group', 'album', 'category', 'hr', 'ctoc', 'film', 'down', 'vote')) && !$this->attach['isimage']) {
			$this->errorcode = -102;
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		if($type == 'album' || $type == 'forum' || $type == 'portal' || $type == 'category' || $type == 'profile' || $type == 'hr' || $type == 'ctoc' || $type == 'film' || $type == 'down' || $type == 'vote') {
			$subdir1 = date('Ym');
			$subdir2 = date('d');
			$subdir = $subdir1.'/'.$subdir2.'/';
	...
	function check_dir_type($type) {
		return !in_array($type, array('forum', 'group', 'album', 'portal', 'common', 'temp', 'category', 'profile', 'hr', 'ctoc', 'film', 'down', 'vote','active')) ? 'temp' : $type;
	}
