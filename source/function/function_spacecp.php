...
function album_update_pic($albumid, $picid=0) {
	...
	$image = new image();
	if($image->Thumb($picsource, 'album/'.$picdir.$albumid.'.jpg', 149, 149, 2)) {  //新空间上线就用它
//	if($image->Thumb($picsource, 'album/'.$picdir.$albumid.'.jpg', 220, 220, 2)) {		//老程序
		$setarr['pic'] = $picdir.$albumid.'.jpg';
		
		//单页所有照片小图
		$image->Thumb($picsource, 'album/'.$picdir.'94/'.$albumid.'.jpg', 94, 94, 2);
		$setarr['pic94'] = $picdir.'94/'.$albumid.'.jpg';
...
function pic_save_xiezhen($FILE, $xiezhenceid, $title, $iswatermark = true, $catid = 0) {

	global $_G, $space;


	if($xiezhenceid<0) $xiezhenceid = 0;
	...
	if($xiezhenceid) {
		$catid = intval($catid);
		$xiezhenceid = album_creat_by_id($xiezhenceid, $catid);
	} else {
		$xiezhenceid = 0;
		$showtip = false;
	}
	...
	if($_G['uid']==11568){
		//艺能新星活动专用
		$result = $image->Thumb($new_name, '', 328, 328, 1);
	}else{
		$result = $image->Thumb($new_name, '', 220, 220, 1);
	}

	$thumb = empty($result)?0:1;		

	if($_GET['tomstar'] == '1'){ //这是明日之星上传活动照片的入口

		//生成 70的小图
		$nowdir = dirname($new_name); //原图所在目录
		$nowdir70 = dirname($new_name).'/70/';

		$arr = explode('/',$new_name);
		$filename = $arr[count($arr)-1];
		$new_name70 =  $nowdir70.$filename;

		$start = strrpos($new_name70,'.','-5');
		$new_name70 = substr($new_name70, $start+2);

		$image->Thumb($new_name, $new_name70, 70, 70, 1);

		$nowdir440 = $nowdir.'/440/';
		$new_name440 =  $nowdir440.$filename;

		$start = strrpos($new_name440,'.','-5');
		$new_name440 = substr($new_name440, $start+2);

		//生成 440的小图
		$image->Thumb($new_name, $new_name440, 580, 440, 1);
	...
	//生成200的小图	
	$nowdir200 = dirname($new_name).'/200/';
	$new_name200 =  $nowdir200.$filename;
	$start = strrpos($new_name200,'.','-5');
	$new_name200 = substr($new_name200, $start+2);
	$image->Thumb($new_name, $new_name200, 200, 200, 3);
	
	//生成80的小图
	$nowdir80 = $nowdir.'/80/';
	$new_name80 = $nowdir80.$filename;
	$start = strrpos($new_name80, '.','-5');
	$new_name80 = substr($new_name80, $start+2);
	$image->Thumb($new_name, $new_name80, 80, 80, 3);
	
	//生成550的小图
	$nowdir550 = $nowdir.'/550/';
	$new_name550 = $nowdir550.$filename;
	$start = strrpos($new_name550, '.','-5');
	$new_name550 = substr($new_name550, $start+2);
	$thumb550bool = $image->Thumb($new_name, $new_name550, 550, 600, 1);


	file_put_contents($file,$text.$out2);
	//liukaiaddend 2013.6.24
	...
	if($thumb550bool){
		$new_name550 = $new_name550;
	}else{
		$new_name550 = 'album/'.$upload->attach['attachment'];
	}
	...
	$setarr = array(
		...
		'remote' => $pic_remote,
		'status' => $pic_status,
		//相册单页的缩略图 liukaiadd 2013.6.24
		'thumb200' => $new_name200,
		'thumb80' => $new_name80,
		'thumb550' => $new_name550
	);
	

	if($_GET['tomstar'] == '1'){ //这是明日之星上传活动照片的入口 添加缩略图	
		$setarr['thumb70'] = $new_name70;
		$setarr['thumb440'] = $new_name440;				
	}
...
function pic_save($FILE, $albumid, $title, $iswatermark = true, $catid = 0) {
	...
	//生成690的小图
	$nowdir690 = $nowdir.'/690/';
	$new_name690 = $nowdir690.$filename;
	$start = strrpos($new_name690, '.','-5');
	$new_name690 = substr($new_name690, $start+2);
	...
	$image->Thumb($new_name, $new_name690, $tw, $th, 1);	
/*	
		//获取源图的尺寸
	$sizeinfo = getimagesize($new_name);
	$w = $sizeinfo[0] > 690 ? 690 : $sizeinfo[0];
	$h = $sizeinfo[1];
	$image->Thumb($new_name, $new_name690, $w, $h, 3);
*/	
	//liukaiaddend 2013.6.24
