...
function setloginstatus($member, $cookietime) {
    ...
    $uname = $_G['username'];
	  $uname=mb_convert_encoding($uname, 'UTF-8', 'GBK');
    $email =$_G['member']['email'];
   //$uface = "http://www.zhangsang.com/images/1.jpg";
    $uface="uc_server/avatar.php?uid=".$_G['uid']."&size=big";
    $ulink =dreferer();
    $expire = "3600";
   $key = "myuestar"; 
    $encode_data = array( 'uid'=>$uid,'uname'=>$uname, 'email'=>$email, 'uface'=>$uface, 'ulink'=>$ulink,'expire'=>$expire );
...
setcookie('syncuyan', authcode(json_encode($encode_data),'ENCODE', $key), time() + 3600, '/', ''); */
