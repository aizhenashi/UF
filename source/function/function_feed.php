...
function mkfeed($feed, $actors=array()) {
	...
	$searchs[] = '{actor}';
	$replaces[] = empty($actors)?"<a href=\"u_$feed[uid]\" target=\"_blank\">$feed[username]</a>":implode(lang('core', 'dot'), $actors);
				...
				foreach($query as $value) {
					if($value['friend'] <= 2) {
						if(empty($setarr['icon'])) {
							...
							$setarr['body_data'] = array(
								'album' => "<a href=\"user.php?mod=view&uid=$value[uid]&do=album&id=$value[albumid]\">$value[albumname]</a>",
								'picnum' => $value['picnum']
							);
						}
						$setarr['image_'.$key] = pic_get($value['filepath'], 'album', $value['thumb'], $value['remote']);
						$setarr['image_'.$key.'_link'] = "user.php?mod=view&uid=$value[uid]&do=album&picid=$value[picid]";
						$key++;
					} else {
						break;
					}
